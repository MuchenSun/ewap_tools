...
destinations = np.loadtxt("/home/msun/workspace/ewap_tools/ewap_dataset/seq_eth/destinations.txt")
...
# load mask 
mask_img = cv2.imread("/home/msun/workspace/ewap_tools/ewap_dataset/seq_eth/map.png", cv2.IMREAD_GRAYSCALE)
...
# start visualization
fig, ax = plt.subplots(1, 2, tight_layout=True, figsize=(16,7))

traj_lag = 40
traj_ahead = 30
for frame in tqdm(full_frame_list):
    ax[0].cla()
    frame_img = cv2.imread("/home/msun/workspace/ewap_tools/ewap_dataset/seq_eth/frames/frame{0:08}.png".format(frame))
    ax[0].imshow(frame_img[...,::-1])
    
    ax[1].cla()
    ax[1].set_xlim(-7.0, 17.0)
    ax[1].set_ylim(14.0, -4.0)
    ax[1].set_aspect('equal')
    ax[1].set_title("Frame: {}".format(frame))
    # ax[1].plot(destinations[:,1], destinations[:,0], linestyle="", marker="x", markersize=10, color="k")
    ...
    for ped in frame_dict[frame]["pedestrians"]:
        ...
        ax[1].plot(state[1], state[0], linestyle='', marker='o', markersize=18, color='C'+str(ped))
    ...
    # plt.pause(0.01)
    plt.savefig("/home/msun/workspace/ewap_tools/ewap_dataset/seq_eth/img/eth_parallel_frame{0:08}.png".format(frame))
